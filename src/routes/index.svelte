...
<template>
	{#if metadata}
		...
		<h1 class="h1">{metadata.title}, {trl("general.created")}: {new Date(metadata.created).toString()}</h1>
	{:else}
		<h1>Open Book</h1>
		<input type="text" id="name" bind:this={bookname} />
		<button on:click={openbook}>{trl("general.open")}</button>
	{/if}
